...
def format_learning_aids(outline: str) -> str:
    """
...
def get_bold_terms(text: str) -> list[tuple[str, int]]:
    """
...
def find_adjacent_terms(terms: list[tuple[str, int]], threshold: int = 20) -> list[list[int]]:
    """
...
def get_color_assignment(cluster: str, shade_index: int) -> str | None:
    """
...
def categorize_terms(terms: list[str], outline: str, logger: CustomLogger) -> dict[str, str]:
    """
    Use LLM to categorize terms into color clusters.
    """
    terms_list = "\n".join([f"- {term}" for term in terms])
    prompt = COLOR_CATEGORIZATION_PROMPT.format(
...
def extract_bold_matches(text: str) -> list[re.Match]:
    """Return all **bold** matches with positions."""
    return list(re.finditer(r'\*\*([^*]+)\*\*', text))


def filter_prohibitive(matches: list[re.Match]) -> list[re.Match]:
...
def build_adjacent_groups(matches: list[re.Match], threshold: int) -> list[list[int]]:
    """Group indices of matches that are close to each other."""
    term_indices = [(m.group(1), m.start()) for m in matches]
    return find_adjacent_terms(term_indices, threshold=threshold)
...
def build_position_color_map(
    ...
    for group_positions in adjacent_groups:
        for shade_idx, term_pos in enumerate(group_positions):
            ...
            color = get_color_assignment(cluster, shade_idx)
...
def replace_terms_once(text: str, position_to_color: dict[int, tuple[str, str]]) -> str:
...
def apply_color_markup(outline: str, logger: CustomLogger) -> str:
    """
    Apply color markup to bold terms with intelligent grouping.
    Each occurrence of a term gets its own color based on position.
    """
    matches = extract_bold_matches(outline)
    if not matches:
        logger.debug("No bold terms found")
        return format_learning_aids(outline)

    filtered = filter_prohibitive(matches)
    if not filtered:
    ...
    categorization = categorize_terms(unique_terms, outline, logger)

    adjacent_groups = build_adjacent_groups(
        filtered, threshold=THREESHOLD_ADJACENT)
    logger.debug(f"Found {len(adjacent_groups)} adjacent term groups")

    position_to_color = build_position_color_map(
        filtered, adjacent_groups, categorization)

    replaced = replace_terms_once(outline, position_to_color)

    formatted_outline = format_learning_aids(replaced)
    logger.debug(
